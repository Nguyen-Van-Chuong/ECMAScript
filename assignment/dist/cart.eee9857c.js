    ...
    if (isPlusButton || isMinusButton) {
        for(let i = 0; i < productsInCart.length; i++){
            if (productsInCart[i].id == e.target.dataset.id) {
                if (isPlusButton) productsInCart[i].count += 1;
                else if (isMinusButton) productsInCart[i].count -= 1;
                productsInCart[i].price = productsInCart[i].basePrice * productsInCart[i].count;
                if (clearBtn) // roductsInCart[i].count -= 1
                console.log("object");
            }
            if (productsInCart[i].count <= 0) productsInCart.splice(i, 1);
        }
        updataa();
    }
});
...
//# sourceMappingURL=cart.eee9857c.js.map
